...
class FlatData extends \Spatie\LaravelData\Data
{
    public function __construct(
        ...
        public ?string $seller_rate_amount,
    ...
    public static function fromModel(Contact $model): self
    {
        ...
        return new self(
            ...
            spouse_salary_gross_income: number_format($data->employment?->toCollection()->firstWhere('type', 'spouse')->monthly_gross_income ?? 0),
            ...
            co_borrower_name: strtoupper(($data->co_borrowers[0]->profile->first_name ?? '').($data->co_borrowers[0]->profile->middle_name ?? '').($data->co_borrowers[0]->profile->last_name ?? '')),
            co_borrower_address: $data->addresses?->toCollection()->firstWhere('type', 'cobo_permanent')->address1 ?? '',
            co_borrower_civil_status: $data->co_borrowers[0]->profile->civil_status ?? '',
            co_borrower_civil_status_lower_case:strtolower($data->co_borrowers[0]->profile->civil_status ?? '') ,
            co_borrower_nationality: $data->co_borrowers[0]->profile->nationality ?? '',
            co_borrower_spouse: strtoupper(($data->co_borrowers[0]->spouse->first_name ?? '').($data->co_borrowers[0]->spouse->middle_name ?? '').($data->co_borrowers[0]->spouse->last_name ?? '')),
            co_borrower_spouse_tin: $data->co_borrowers[0]->spouse_tin ?? '',
            co_borrower_tin: $data->co_borrowers[0]->tin ?? '',
            client_id_aif: $data->order->aif->client_id ?? '',
            aif_address: $data->order->aif->address ?? '',
            aif_name: strtoupper(($data->order->aif->profile->first_name ?? '').($data->order->aif->profile->middle_name ?? '').($data->order->aif->profile->last_name ?? '')),
            aif_last_name: strtoupper($data->order->aif->profile->last_name ?? ''),
            aif_first_name: strtoupper($data->order->aif->profile->first_name ?? ''),
            aif_middle_name: strtoupper($data->order->aif->profile->middle_name ?? ''),
            aif_extension_name: strtoupper($data->order->aif->profile->name_suffix ?? ''),
            aif_unit_lot: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->unit ?? '',
            aif_street: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->street ?? '',
            aif_subdivision: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->subdivision ?? '',
            aif_barangay: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->sublocality ?? '',
            aif_city: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->locality ?? '',
            aif_province: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->administrative_area ?? '',
            aif_zip_code: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->postal_code ?? '',
            aif_length_of_stay: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->length_of_stay ?? '',
            aif_ownership_type: $data->addresses?->toCollection()->firstWhere('type', 'aif_permanent')->ownership ?? '',
            aif_birthday: $data->order->aif->profile->date_of_birth ?? '',
            aif_age: $data->order->aif->profile->date_of_birth ? Carbon::parse($data->order->aif->profile->date_of_birth)->age : 0,
            aif_gender: $data->order->aif->profile->sex ?? '',
            aif_civil_status: $data->order->aif->profile->civil_status ?? '',
            aif_civil_status_lower_case: strtolower($data->order->aif->profile->civil_status ?? ''),
            aif_nationality: $data->order->aif->profile->nationality ?? '',
            aif_residence_landline: $data->order->aif->profile->landline ?? '',
            aif_primary_contact_number: $data->order->aif->profile->mobile ?? '',
            aif_email: $data->order->aif->profile->email ?? '',
            aif_relationship_to_buyer: $data->order->aif->profile->relationship_to_buyer ?? '',
            aif_account_name: strtoupper($data->order->aif->account_name ?? ''),
            aif_username_or_email: $data->order->aif->username_or_email ?? '',
            aif_sss: $data->employment?->toCollection()->firstWhere('type', 'aif')->id->sss ?? '',
            aif_pagibig: $data->employment?->toCollection()->firstWhere('type', 'aif')->id->pagibig ?? '',
            aif_tin: $data->employment?->toCollection()->firstWhere('type', 'aif')->id->tin ?? '',
            aif_passport: $data->order->aif->profile->passport ?? '',
            aif_date_issued:$data->order->aif->profile->date_issued ?? '',
            aif_place_issued:$data->order->aif->profile->place_issued ?? '',
            aif_employer_name: strtoupper($data->employment?->toCollection()->firstWhere('type', 'aif')->employer->name ?? ''),
            aif_employer_address: $data->employment?->toCollection()->firstWhere('type', 'aif')->employer->address->full_address ?? '',
            aif_employer_type: $data->employment?->toCollection()->firstWhere('type', 'aif')->employer->type ?? '',
            aif_employment_status: $data->employment?->toCollection()->firstWhere('type', 'aif')->employment_status ?? '',
            aif_position: $data->employment?->toCollection()->firstWhere('type', 'aif')->current_position ?? '',
            aif_industry: $data->employment?->toCollection()->firstWhere('type', 'aif')->employer->industry ?? '',
            aif_salary_gross_income: $data->employment?->toCollection()->firstWhere('type', 'aif')->monthly_gross_salary ?? '',
            aif_company_phone_number: $data->employment?->toCollection()->firstWhere('type', 'aif')->employer->contact_no ?? '',
            aif_fax: $data->employment?->toCollection()->firstWhere('type', 'aif')->employer->fax ?? '',
            aif_company_email: $data->employment?->toCollection()->firstWhere('type', 'aif')->employer->email ?? '',
            ...
            seller_rate: $data->order->seller->rate ?? '',
            seller_rate_amount: number_format($data->order->seller->rate_amount ?? 0, 2),
            ...
            equity_1_interest_rate: $data->order->equity_1_interest_rate ?? '',
            ...
            bp_1_interest_rate: $data->order->bp_1_interest_rate ?? '',
            ...
            exec_position: 'Chief Seller Officer',
            ...
            client_id_co_borrower: $data->co_borrowers[0]->id ?? '',
